...
if not os.path.exists(FILE_DB):
    pd.DataFrame(columns=[
        ...
        "tanggal",
        "file_path"
    ]).to_excel(FILE_DB, index=False)

def load_database():
    return pd.read_excel(FILE_DB, engine="openpyxl")
...
# ================= BUAT SURAT =================
    ...
    with st.container():
        ...
        with col1:
            nama = st.text_input("Nama Lembaga", key='nama')
            npwp = st.text_input("NPWP", key='npwp')

        with col2:
            alamat = st.text_area("Alamat", key='alamat')
    ...
    with st.container():
        ...
        with col1:
            notaris_pendirian = st.text_input("Notaris Pendirian", key='notaris_pendirian')
            nomor_akte_pendirian = st.text_input("Nomor/Tanggal Akta Pendirian", key='nomor_akte_pendirian')

        with col2:
            notaris_perubahan = st.text_input("Notaris Perubahan Terakhir", key='notaris_perubahan')
            nomor_akte_perubahan = st.text_input("Nomor/Tanggal Perubahan", key='nomor_akte_perubahan')
    ...
    with st.container():
        ...
        with col1:
            stp_tanggal = st.text_input("Tanggal STP Terakhir", key='stp_tanggal')

        with col2:
            stp_nomor = st.text_input("Nomor STP Terakhir", key='stp_nomor')
    ...
    with st.container():
        ...
        with col1:
            status = st.selectbox("Status Organisasi", ["Pusat", "Cabang"], key='status')

        with col2:
            lingkup = st.selectbox("Lingkup Wilayah Kerja", ["Provinsi", "Kabupaten"], key='lingkup')
    
    # =============== JENIS LKS ==============
    with st.container():
        st.subheader("Jenis LKS")
        jenis_lks = st.selectbox("Jenis LKS", ["Anak", "Lansia", "Psikotik"], key='jenis_lks')
    # ================= USAHA =================
    with st.container():
        st.subheader("💼 Kegiatan Usaha")

        usaha_jalan = st.text_area("Usaha Kesejahteraan Sosial yang Sedang Dilaksanakan", key='usaha_jalan')
        usaha_rencana = st.text_area("Rencana Usaha Kesejahteraan Sosial", key='usaha_rencana')
    ...
    with st.container():
        ...
        with col1:
            dasar_tanggal = st.text_input("Tanggal", key='dasar_tanggal')

        with col2:
            dasar_nomor = st.text_input("Nomor", key='dasar_nomor')
    ...
    with st.container():
        ...
        with col1:
            tanggal_permohonan = st.text_input("Tanggal", key='tanggal_permohonan')

        with col2:
            nomor_permohonan = st.text_input("nomor", key='nomor_permohonan')
            ...
            with col1:
                if st.button("✅ Lanjut Generate"):
                    try:
                        ...
                        df_final.to_excel(FILE_DB, index=False)
